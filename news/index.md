...
- Removed the `aniframe_kin2d` and `aniframe_kin3d` classes.
  ...
  test in any package — and the dimensionality they encoded is already
  carried by the `coordinate_system` metadata field, so they were a
  second source of truth that could only ever disagree with the first.
  Kinematics output still carries `aniframe_kin`, which is the class the
  `summarise_*()` functions dispatch on.
