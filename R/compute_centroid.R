...
#'
#' @return An anipoint containing only the summary member. Coordinate values
...
compute_centroid <- function(
  ...
  space_cols <- anicore::get_variables(data, "where", "position")
  ...
  centroid <- data |>
    ...
    dplyr::summarise(
      dplyr::across(
        dplyr::all_of(unname(space_cols)),
        \(v) mean(v, na.rm = TRUE)
      ),
...
#' @param source The anipoint it came from.
#' @param space_cols The spatial columns `derived` carries.
#'
#' @return `derived` as an anipoint, declared as `source` was.
#' @keywords internal
